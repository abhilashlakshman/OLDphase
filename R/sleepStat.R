...
sleepStat <- function(input, sleep.def = c(5), t.cycle = 24, photoperiod = 12) {
  ...
  colnames(output) <- c("Channel", "Day.BoutNumber", "Day.BoutDuration", "Day.Latency",
                        "Day.Total",
                        "Night.BoutNumber", "Night.BoutDuration",
  ...
  day.total <- matrix(NA, nrow = 32, ncol = n.days)
  ...
  night.total <- matrix(NA, nrow = 32, ncol = n.days)
  ...
  if (length(sleep.def) == 1) {
    for (i in 1:length(cyc.wise.split)) {
      ...
      for (j in 1:length(day[1,])) {
        ...
        day.total[j,i] <- sum(ddf.d$lengths)
        ...
        night.total[j,i] <- sum(ddf.n$lengths)
    ...
    for (i in 1:length(raw[1,])) {
      ...
      output[i,"Day.Total"] = mean(day.total[i,], na.rm = T)
      output[i,"Night.Total"] = mean(night.total[i,], na.rm = T)
    }
